...
GitHub release AARs. The fork changes are limited to reproducible build,
source packaging, and publication support on top of the upstream `1.18`
source tags; they do not contain Chimahon animated-scene application code.
...
release tag, so preview and benchmark CI remain usable before a tagged release.

## Application release status

Native compliance is complete. Application publication remains independently
blocked by `immersion-stats-release-validation.json` until its reviewed device
and release evidence is complete. A missing release OAuth or signing secret
also prevents the GitHub Actions release build. Neither condition is waived by
this native audit.
