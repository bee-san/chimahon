...
class AnkiInventorySynchronizer(
    ...
    private fun buildItems(
        ...
        val candidates = inventory.notes.mapNotNull { note ->
            ...
            val reading = configuration.readingField
                ?.let(note.fields::get)
                ?.stripAnkiMarkup()
                ?.trim()
                .orEmpty()
            val normalizedWord = ImmersionLexemeNormalizer.normalizeHeadword(
                expression,
                configuration.languageTag,
            )
            val normalizedReading = ImmersionLexemeNormalizer.normalizeReading(reading)
        ...
        val readingsByWord = candidates.groupBy(Candidate::normalizedWord)
            .mapValues { (_, values) ->
                values.map(Candidate::normalizedReading).filter(String::isNotBlank).distinct().size
            }

        return candidates.flatMap { candidate ->
            val ambiguityCount = readingsByWord[candidate.normalizedWord] ?: 0
            val matchConfidence = when {
                candidate.normalizedReading.isNotBlank() -> AnkiMatchConfidence.READING_AWARE
                ambiguityCount > 1 -> AnkiMatchConfidence.AMBIGUOUS
                else -> AnkiMatchConfidence.HEADWORD_ONLY
            }
...
data class AnkiCoverage(
    ...
    val coveredReadingAware: Long,
    val coveredHeadwordOrCharacter: Long,
) {
    init {
        require(encountered >= 0)
        require(coveredReadingAware >= 0)
        require(coveredHeadwordOrCharacter >= 0)
        require(coveredReadingAware <= coveredHeadwordOrCharacter)
        require(coveredHeadwordOrCharacter <= encountered)
    }
}
...
class AnkiKnownnessResolver(
    ...
    suspend fun word(
        ...
        if (!enabled) return AnkiKnownness(MaturityTier.UNAVAILABLE, null, 0, null)
        ...
        val grouped = items.groupBy(ImmersionAnkiItem::noteId)
        val noteTiers = grouped.values.map { cards ->
    ...
    suspend fun character(
        ...
        return AnkiKnownness(tier, null, 0, snapshot.completedAtEpochMillis)
    }
}
