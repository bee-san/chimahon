...
class AnkiInventoryTest {
    ...
    fun `successful refresh atomically maps reading-aware words characters and mixed cards`() = runTest {
        ...
        resolver.character(true, PROFILE, UnicodeCodePoint('生'.code)).tier shouldBe MaturityTier.MATURE
        resolver.character(
            enabled = true,
            profileId = PROFILE,
    ...
    fun `disabled and unavailable integration never masquerade as unknown knowledge`() = runTest {
        ...
        AnkiKnownnessResolver(repository)
            .character(false, PROFILE, UnicodeCodePoint('猫'.code)).tier shouldBe
