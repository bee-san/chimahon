...
data class AnimeTrackInfoDialogHomeScreen(
    ...
    private class Model(
        ...
        fun registerEnhancedTracking(item: AnimeTrackItem) {
            ...
            screenModelScope.launchNonCancellable {
                ...
                try {
                    ...
                    (item.tracker as AnimeTracker).register(matchResult, animeId)
                } catch (e: Exception) {
        ...
        private fun List<AnimeTrack>.mapToTrackItem(): List<AnimeTrackItem> {
            val loggedInTrackers = Injekt.get<TrackerManager>().loggedInAnimeTrackers()
...
private data class TrackStatusSelectorScreen(
    ...
    private class Model(
        ...
        fun getSelections(): Map<Long, StringResource?> {
            return (tracker as AnimeTracker).getStatusListAnime().associateWith {
        ...
        fun setStatus() {
            screenModelScope.launchNonCancellable {
                (tracker as AnimeTracker).setRemoteAnimeStatus(track.toDbTrack(), state.value.selection)
            }
        }
...
private data class TrackEpisodeSelectorScreen(
    ...
    private class Model(
        ...
        fun setEpisode() {
            screenModelScope.launchNonCancellable {
                (tracker as AnimeTracker).setRemoteLastEpisodeSeen(
                    track.toDbTrack(),
...
private data class TrackScoreSelectorScreen(
    ...
    private class Model(
        private val track: DbAnimeTrack,
        private val tracker: Tracker,
    ) : StateScreenModel<Model.State>(State((tracker as AnimeTracker).displayScore(track))) {

        fun getSelections(): ImmutableList<String> {
            return (tracker as AnimeTracker).getScoreList()
        }
        ...
        fun setScore() {
            screenModelScope.launchNonCancellable {
                (tracker as AnimeTracker).setRemoteScore(track.toDbTrack(), state.value.selection)
            }
        }
...
@Composable
private fun AnimeTrackEpisodeSelector(
    selection: Int,
    ...
    AlertDialogContent(
        ...
        title = { Text(text = stringResource(MR.strings.episodes)) },
        text = {
...
private data class TrackDateSelectorScreen(
    ...
    private class Model(
        ...
        fun setDate(millis: Long) {
            ...
            screenModelScope.launchNonCancellable {
                if (start) {
                    (tracker as AnimeTracker).setRemoteStartDate(track.toDbTrack(), localMillis)
                } else {
                    (tracker as AnimeTracker).setRemoteFinishDate(track.toDbTrack(), localMillis)
                }
            }
        }
...
private data class TrackDateRemoverScreen(
    ...
    private class Model(
        ...
        fun removeDate() {
            screenModelScope.launchNonCancellable {
                if (start) {
                    (tracker as AnimeTracker).setRemoteStartDate(track.toDbTrack(), 0)
                } else {
                    (tracker as AnimeTracker).setRemoteFinishDate(track.toDbTrack(), 0)
                }
            }
        }
    }
...
data class TrackServiceSearchScreen(
    ...
    private class Model(
        ...
        fun trackingSearch(query: String) {
            screenModelScope.launch {
                ...
                val result = withIOContext {
                    try {
                        val results = (tracker as AnimeTracker).searchAnime(query)
        ...
        fun registerTracking(item: AnimeTrackSearch) {
            screenModelScope.launchNonCancellable { (tracker as AnimeTracker).register(item, animeId) }
        }
